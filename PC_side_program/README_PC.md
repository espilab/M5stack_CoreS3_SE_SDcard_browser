...
```
python m5sd_synccopy.py <PC側パス> <ESP32側フォルダ> [-d]
```
...
2. コピー先フォルダが存在しなければ作成（親フォルダも再帰的に作成）
3. PC 側フォルダをサブディレクトリも含めて再帰的に走査
4. ファイルごとにタイムスタンプを比較し、PC 側が新しい場合のみアップロード
5. `-d` オプション指定時：ESP32 側にあって PC 側にないファイル・ディレクトリを削除
